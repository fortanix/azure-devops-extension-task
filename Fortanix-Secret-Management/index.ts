...
async function run() {
  try {
    ...
    console.log("[INFO] Inputs have been received");
    const SOBJECT_EXPORT_PATH = "/crypto/v1/keys/export"
    fetchSecurityObject(FORTANIX_API_ENDPOINT, SOBJECT_EXPORT_PATH, FORTANIX_API_KEY, FORTANIX_SECURITY_OBJECT_NAME, FORTANIX_SECURITY_OBJECT);
  } catch (err: any) {
    tl.setResult(tl.TaskResult.Failed, err.message);
  }
}

function fetchSecurityObject(FORTANIX_API_ENDPOINT: string, SOBJECT_EXPORT_PATH: string, FORTANIX_API_KEY: string, NAME: string, FORTANIX_SECURITY_OBJECT: string) {
  const data = JSON.stringify({
    name: NAME
  });
  const options = {
    hostname: FORTANIX_API_ENDPOINT,
    path: SOBJECT_EXPORT_PATH,
    method: 'POST',
    headers: {
